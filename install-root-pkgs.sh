...
# microsoft repos
if ! [ -f /etc/apt/trusted.gpg.d/microsoft-keyring.gpg ]; then
  echo -e "\e[34mAdd Microsoft keyring to /etc/apt/trusted.gpg.d/microsoft-keyring.gpg.\e[0m"
...
if ! hash az 2>/dev/null; then
  ...
  addSourcesList "https://packages.microsoft.com/repos/azure-cli/ `lsb_release -cs` main" microsoft azure-cli
